# CIS 410: Cybersecurity Automation


## About Me

- **Name:** [Yalew Wakjira]

- **GitHub Username:** [yalew-a]

- **Major:** [Cybersecurity]

- **Semester:** [Spring]


## About This Course

This repository is my CIS 410: Cybersecurity Automation portfolio. In this course, I am learning about secure software development, CI/CD pipelines, containerization, infrastructure as code, and cloud deployment. These skills help me understand how to automate security processes and protect systems in real-world environments.


## What I Hope to Learn

- How to use GitHub for version control and collaboration
- How to work with Azure and deploy infrastructure using Infrastructure as Code (IaC)
- How DevSecOps integrates security into the software development lifecycle (SDLC)

## Fun Fact

I am passionate about cybersecurity and currently working on building my skills for a future career in network security and system protection.

## My Week 2 Infrastructure

 

Runner VM: wakjira-runner

Runner IP: 192.168.1.186

Pipeline status: [![Hello Pipeline](https://github.com/YOUR-USERNAME/cis-410-cybersecurity-automation/actions/workflows/hello-pipeline.yml/badge.svg)](https://github.com/YOUR-USERNAME/cis-410-cybersecurity-automation/actions/workflows/hello-pipeline.yml)